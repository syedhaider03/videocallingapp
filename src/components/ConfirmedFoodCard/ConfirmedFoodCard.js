...
import { HP, WP, isX, cake, cal, arrowRgrey, group, banner, crossgradient, staryellow, stargrey, CFL,RF, fonts, colors, floatPad } from '../../utils/contants';
import Stars from 'react-native-stars';

const ConfirmedFoodCard = (props) => {
    return (
        <TouchableOpacity onPress={props.onPress} style={styles.container}>
            <View style={{ flexDirection: 'row', marginBottom: HP(1.2) }}>
                ...
                <View>
                    {
                        props.dishes.map(dish => (
                            <Text style={styles.dishText}><Text style={{ fontFamily: fonts.med }}>{dish.quantity}</Text> {CFL(dish.name)}</Text>
                        ))
                    }
                </View>
...
export default ConfirmedFoodCard;